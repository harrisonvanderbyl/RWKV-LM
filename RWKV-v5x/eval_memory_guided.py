...
token_ban = [187] #[on_token] # ban the generation of some tokens
...
reply_suffix = "\n```\n"
prompt_prefix_state = None
# Process the prompt prefix
for x in model.encode(prompt_prefix):
    prompt_prefix_logits, prompt_prefix_state = model.forward([x], prompt_prefix_state)
...
def validate_model(token_count):
    ...
    for i in range(len(target_tokens)):
        ...
        top_token = sorted_indices[0].cpu().item()
        # print(model.decode([top_token]),
        top_prob = sorted_probs[0].cpu().item()
